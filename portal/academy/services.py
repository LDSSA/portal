...
def perform_grading_local(user, unit):
    ...
    process = subprocess.run(
        [
            ...
            "portal",
            "--env",
            f"LDSA_TOKEN={params['token']}",
            "--env",
            f"LDSA_GRADING_URL={params['grading_url']}",
            "--env",
            f"LDSA_CHECKSUM_URL={params['checksum_url']}",
            "--env",
            f"DEPLOY_KEY={key}",
            "--env",
            f"CODENAME={unit.code}",
            "--env",
            f"USERNAME={user.username}",
            "--env",
            f"REPO_NAME={settings.STUDENT_REPO_NAME}",
            f"{unit.code.lower()}",
        ]
    )
...
def perform_grading_production(user, unit):
    ...
    process = subprocess.Popen(
        [
            "kubectl",
            "run",
            f"{unit.code.lower()}-{user.username.lower()}-{id_}",
            "--restart=Never",
