...
for dp, row in zip(models.Datapoint.objects.filter(simulator=sim).order_by('id').all()[:200], data[:200]):
    print(json.loads(dp.data)['observation_id'], row['data']['observation_id'])

for dp, row in zip(models.Datapoint.objects.filter(simulator=sim).order_by('id').iterator(), data):
